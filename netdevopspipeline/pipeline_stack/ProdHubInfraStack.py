...
class ProdHubStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, cidr: str, dr_cidr: str, region: str, **kwargs) -> None:
        ...
        ram_share = ram.CfnResourceShare(
            ...
            resource_arns=[f"arn:aws:ec2:{region}:{netdevopspipeline.accounts.Prod_Hub_Primary_ENV.account}:transit-gateway/{self.transit_gateway.attr_id}"],
            principals=[f"{netdevopspipeline.accounts.Prod_Spoke_Primary_ENV.account}"],
            name="tgw-share",
        )
